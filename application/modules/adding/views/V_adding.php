<div class="main-wrapper" style="background-image">
            <div class="main">
                <div class="main-inner">


                    <div class="content">
                        <div class="content-title">
                            <div class="container">
                                    <img src="assets/img/icon.png" height="23px"></img>
                                <h1>Malang</h1>

                                <ul class="breadcrumb">
                                    <h7 style="">Tambahkan pilihan yang ingin anda kunjungi</h7>

                                </ul>
                                <!-- /.breadcrumb -->
                            </div>
                            <!-- /.container -->
                        </div>
                        <!-- /.content-title -->
                    </br>
                </br>
                        <div class="container">
                        <div class="row">
                            <div class="col-12">
                                            <center><h2>Pilih Apa Yang ingin Anda Tambahkan</h2></center>
                                            <div class="filter filter-white">
                                                    <div class="row">
                                                
                                            
                                                <div class="col-md-8">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control" placeholder="Nama Tujuan">
                                                    </div><!-- /.form-group -->
                                                </div><!-- /.col-* -->
                                            
                                                <div class="col-md-2">
                                                    <a href="listings_map2"><div class="form-group-btn form-group-btn-placeholder-gap">
                                                        <button type="button" class="btn btn-primary btn-block">Cari</button></a>
                                                    </div><!-- /.form-group -->		
                                                </div><!-- /.col-* -->			
                                            </div><!-- /.row --> 
                                                </div><!-- /.filter -->
                                            
                                                <div class="row">
                                                    

                            </div>
                            </div>
                        </div>
                    </br>
                    
                    <div class="row">
                        <div class="col-md-7"> 
                            <div class="container">
                                <div class="page-title">
                                        
                                </br>
                            </br>
                        </br>
                                    <div class="cards-wrapper">
                                            <div class="row">
                                                    <div class="col-xs-12 col-sm-6 col-lg-4">
                                                        <div class="card">
                                                            <div class="card-image small no-line" style="background-image: url('assets/img/wisata/borobudur.jpg');">
                                                                <a href="addwisata"></a>
                                    
                                                                <div class="card-image-icon">
                                                                    <i class="md-icon">room</i>
                                                                </div><!-- /.card-image-icon -->
                                    
                                                                <div class="card-image-count">
                                                                    <strong>30</strong>
                                                                    <span>item</span>
                                                                </div><!-- /.card-image-count -->
                                                            </div><!-- /.card-image -->
                                    
                                                            <div class="card-content">
                                                                <h2>
                                                                    <a href="addwisata">Wisata</a>
                                    
                                                                    <a href="addwisata" class="btn btn-transparent">Show All</a>
                                                                </h2>
                                                            </div><!-- /.card-content -->
                                                        </div><!-- /.card -->
                                                    </div><!-- /.col-* -->
                                                
                                                    <div class="col-xs-12 col-sm-6 col-lg-4">
                                                        <div class="card">
                                                            <div class="card-image small no-line" style="background-image: url('assets/img/kuliner.png');">
                                                                <a href="addkuliner"></a>
                                    
                                                                <div class="card-image-icon">
                                                                    <i class="md-icon">restaurant_menu</i>
                                                                </div><!-- /.card-image-icon -->
                                    
                                                                <div class="card-image-count">
                                                                    <strong>43</strong>
                                                                    <span>items</span>
                                                                </div><!-- /.card-image-count -->
                                                            </div><!-- /.card-image -->
                                    
                                                            <div class="card-content">
                                                                <h2>
                                                                    <a href="addkuliner">Kuliner</a>
                                    
                                                                    <a href="addkuliner" class="btn btn-transparent">Show All</a>
                                                                </h2>
                                                            </div><!-- /.card-content -->
                                                        </div><!-- /.card -->
                                                    </div><!-- /.col-* -->
                                                
                                                    <div class="col-xs-12 col-sm-6 col-lg-4">
                                                        <div class="card">
                                                            <div class="card-image small no-line" style="background-image: url('assets/img/oleh/legong3.jpg');">
                                                                <a href="addoleh"></a>
                                    
                                                                <div class="card-image-icon">
                                                                    <i class="md-icon">event</i>
                                                                </div><!-- /.card-image-icon -->
                                    
                                                                <div class="card-image-count">
                                                                    <strong>92</strong>
                                                                    <span>items</span>
                                                                </div><!-- /.card-image-count -->
                                                            </div><!-- /.card-image -->
                                    
                                                            <div class="card-content">
                                                                <h2>
                                                                    <a href="addoleh">Oleh-oleh</a>
                                    
                                                                    <a href="addoleh" class="btn btn-transparent">Show All</a>
                                                                </h2>
                                                            </div><!-- /.card-content -->
                                                        </div><!-- /.card -->
                                                    </div><!-- /.col-* -->
                                            </div><!-- /.row -->
                                        </div><!-- /.cards-wrapper -->
                                        <a href="listings"><div class="btn btn-primary" type="button">Kembali</div></a>
                                </div><!-- /.page-title -->
                                </div><!--container-->
                            </div>
                            <div class="col-lg-5">
                                    <div class="sidebar">
                                            <div class="admin-box no-padding">
                                                    <div class="widget">
                                                            <center><h2 class="widgettitle">Daftar Destinasi Anda</h2></center>
                                                    <table class="table small-header">
                                                        <thead>
                                                            <tr>
                                                                <th>No</th>
                                                                <th>Nama Destinasi</th>
                                                                <th>Tindakan</th>
                                                                
                                                            </tr>
                                                        </thead>
                                                
                                                        <tbody>
                                                            
                                                                <tr>
                                                                    <td>1</td>
                                                                    <td>Bromo Market</td>		
                                                                    <td class="min-width">
                                                                        <a href="#"><i class="md-icon">delete</i></a>
                                                                    </td>			
                                                                </tr>
                                                            
                                                                <tr>
                                                                    
                                                                    <td>2</td>
                                                                    <td>Flora Wisata San Terra</td>	
                                                                            <td class="min-width">
                                                                                    <a href="#"><i class="md-icon">delete</i></a>
                                                                                </td>		
                                                                </tr>
                                                                <tr>
                                                                        <td>3</td>
                                                                        <td>Depot Makan Khas Malang</td>	
                                                                        <td class="min-width">
                                                                                <a href="#"><i class="md-icon">delete</i></a>
                                                                            </td>		
                                                                    </tr>
                                                            
                                                                
                                                            
                                                        </tbody>
                                                    </table><!-- /.table -->
                                                </div><!-- /.admin-box -->
                                                <div class="container">
                                                <center><a href="jadwal"><div class="btn btn-success" type="button">Atur Jadwal</div></a></center>
                                                </br>
                                    </div>
                                </div>
                                    <!-- /.sidebar -->
                                </div>
                                <!-- /.col-* -->
                                </div>
                                </div>
                            </div>
                    </div>
                    <!-- /.content -->
                </div>
                <!-- /.main-inner -->
            </div>
            <!-- /.main -->
        </div>

        <div id="1" class="modal" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <center><h4 class="modal-title">Apakah anda ingin menambahkan ini?</h4></center>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
                      <a href="adding"><button type="button" class="btn btn-primary">Iya</button></a>
                    </div>
                  </div>
                </div>
              </div>